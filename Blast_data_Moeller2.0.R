...
OVMean <- sqldf("
  SELECT
    player_name,
    player_id,
    AVG(early_connection) as AvgEarlyConnection,
    AVG(body_tilt_angle) as AvgBodyTiltAngle,
    AVG(bat_path_angle) as AvgBatPathAngle,
    AVG(rotational_acceleration) as AvgRotationalAcceleration,
    AVG(time_to_contact) as AvgTimeToContact,
    AVG(body_rotation) as AvgBodyRotation,
    AVG(peak_speed) as AvgPeakSpeed
  FROM
    df
  GROUP BY
    player_id
